...
func (h *Handler) handleAccountFailure(account *config.Account, err error) {
	...
	case isOverageErrorMessage(errMsg):
		...
		h.pool.RecordFailure(account.ID, pool.FailureReasonQuotaExhausted)
	case isQuotaErrorMessage(errMsg):
		h.pool.RecordFailure(account.ID, pool.FailureReasonRateLimited)
	...
	default:
		h.pool.RecordFailure(account.ID, pool.FailureReasonUnknown)
	}
}
