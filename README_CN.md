...
- 面向 sub2api 兼容 Opus 4.7 生成请求的 StableDownstream 模式：下游不暴露 `429`、`502`、`503`，内部抑制原因写入请求日志。
...
- StableDownstream 模式可以让 sub2api-facing 生成响应保持合法响应体和 HTTP `200`，同时在 Kiro-Go 请求日志里记录被内部抑制的可重试失败。
