...
- 当前 Claude Code 兼容矩阵见 [`docs/claude-code-compatibility-matrix.md`](docs/claude-code-compatibility-matrix.md)。矩阵会区分 Claude Code 可用的本地兼容行为，以及 Kiro 上游不可证明、估算或模拟的官方 Anthropic parity。
- 可用这些管理端 readiness API 排查实时状态：`/admin/api/claude-code/compat`、`/admin/api/claude-code/readiness`、`/admin/api/claude-code/model-readiness?model=claude-opus-4-7`。
- 大账号池运维可用这些只读/诊断 API，避免直接消耗生成流量：`/admin/api/auth/credentials/validate`、`/admin/api/accounts/{id}/diagnostics`、`/admin/api/scheduler/preview?model=claude-opus-4-7`、`/admin/api/fleet/readiness`、`/admin/api/websearch/diagnostics`。
